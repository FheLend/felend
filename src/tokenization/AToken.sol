...
contract AToken is ERC20 {
    ...
    **/
     ...
     **/
    function transfer(address _to, uint256 _amount) public override whenTransferAllowed(msg.sender, _amount) returns (bool) {
        executeTransferInternal(msg.sender, _to, _amount);
        return true;
    }
    ...
    **/
    function redeem(inEuint128 calldata _amount) external {
        euint128 amountRedeem = FHE.asEuint128(_amount);
        ...
        currentBalance,
        balanceIncrease,
        index) = cumulateBalanceInternal(msg.sender);

        //if amount is equal to uint(-1), the user wants to redeem everything
        if(amountToRedeem == UINT_MAX_VALUE){
            amountToRedeem = currentBalance;
        }

        require(currentBalance >= amountToRedeem, "available balance must greater than redeem amount");
        ...
        // euint32 encryptedAmountToRedeem = FHE.asEuint32(amountToRedeem);
     ...
     * @param _encryptedAmount the amount of tokens to mint
     */
    function mintOnDeposit(address _account, euint128 _encryptedAmount) external { // TODO: restrict here: onlyLendingPool or something
