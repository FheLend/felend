...
contract Pool is ReentrancyGuard, VersionedInitializable { 
    ...
    * @param _encryptedAmount the amount to be deposited
    * @param _timestamp the timestamp of the action
    **/
    event Deposit(
        address indexed _reserve,
        address indexed _user,
        euint128 _encryptedAmount,
        uint256 _timestamp
    );
    ...
    **/
    modifier onlyAmountGreaterThanZero(inEuint128 memory _amount) {
    ...
    * @param _encryptedAmount the amount to be deposited
    **/
    function deposit(address _reserve, inEuint128 calldata _encryptedAmount)
        external
        payable
        nonReentrant
        onlyActiveReserve(_reserve)
        onlyUnfreezedReserve(_reserve)
        onlyAmountGreaterThanZero(_encryptedAmount)
    {
        AToken aToken = AToken(core.getReserveATokenAddress(_reserve));

        bool isFirstDeposit = aToken.balanceOf(msg.sender) == 0;
        euint128 depositAmount = FHE.asEuint128(_encryptedAmount);

        core.updateStateOnDeposit(_reserve, msg.sender, depositAmount , isFirstDeposit);

        //minting AToken to user 1:1 with the specific exchange rate
        aToken.mintOnDeposit(msg.sender, depositAmount);

        //transfer to the core contract
        core.transferToReserve{value:msg.value}(_reserve, payable(msg.sender), depositAmount);

        //solium-disable-next-line
        emit Deposit(_reserve, msg.sender, depositAmount, block.timestamp);
    ...
    * @param _reserve the address of the reserve
    * @param _encryptedAmount the amount to be borrowed
    ...
    **/
    ...
    /**
    * @notice repays a borrow on the specific reserve, for the specified amount (or for the whole amount, if uint256.max is specified).
    * @dev the target user is defined by _onBehalfOf. If there is no repayment on behalf of another account,
    * _onBehalfOf must be equal to msg.sender.
    * @param _reserve the address of the reserve on which the user borrowed
    * @param _amount the amount to repay, or uint256.max if the user wants to repay everything
    * @param _onBehalfOf the address for which msg.sender is repaying.
    **/

    struct RepayLocalVars {
        euint128 principalBorrowBalance;
    ...
    function repay(address _reserve, inEuint128 calldata _encryptedAmount, address payable _onBehalfOf)
        external
        payable
        nonReentrant
        onlyActiveReserve(_reserve)
        onlyAmountGreaterThanZero(_encryptedAmount)
    {
        ...
        // vars.principalBorrowBalance = FHE.asEuint32(principalBorrowBalance);
        ...
        euint128 repayAmount = FHE.asEuint128(_encryptedAmount);
        uint256 _amount = FHE.decrypt(repayAmount);
        ...
        euint128 encryptedPaybackAmountMinusFees = FHE.asEuint128(vars.paybackAmountMinusFees);
        core.transferToReserve{value: vars.isETH ? msg.value - vars.originationFee : 0}(
            _reserve,
            payable(msg.sender),
            encryptedPaybackAmountMinusFees
        );
    ...
    * this is to ensure that the protocol is not abused, and the user is paying a fair
    ...
    **/
        ...
        onlyActiveReserve(_reserve)
    {
        ...
        uint256 rebalanceDownRateThreshold = reserveCurrentStableRate * (WadRayMath.ray() + parametersProvider.getRebalanceDownRateDelta());
    ...
    **/
    function requireAmountGreaterThanZeroInternal(inEuint128 memory _amount) internal pure {
        euint128 zero = FHE.asEuint128(0);
        euint128 amount = FHE.asEuint128(_amount);
        FHE.req(amount.gt(zero));
    }
}
