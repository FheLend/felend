...
contract LendingPoolLiquidationManager is ReentrancyGuard, VersionedInitializable {
    ...
    **/
    function liquidationCall(
        ...
        euint128 actualAmountToLiquidate = FHE.asEuint128(vars.actualAmountToLiquidate);
        core.transferToReserve{value:msg.value}(_reserve, payable(msg.sender), actualAmountToLiquidate);
    ...
    * @return collateralAmount the maximum amount that is possible to liquidated given all the liquidation constraints (user balance, close factor) and
    * the purchase amount
    **/
    function calculateAvailableCollateralToLiquidate(
        ...
        vars.maxAmountCollateralToLiquidate = vars
            .principalCurrencyPrice * _purchaseAmount / vars.collateralPrice * vars.liquidationBonus / 100;

        if (vars.maxAmountCollateralToLiquidate > _userCollateralBalance) {
            collateralAmount = _userCollateralBalance;
            principalAmountNeeded = vars
                .collateralPrice * collateralAmount / vars.principalCurrencyPrice * 100 / vars.liquidationBonus;
        } else {
