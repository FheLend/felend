...
contract LendingPoolDataProvider is VersionedInitializable {
    ...
    * @return totalLiquidityBalanceETH the total liquidity, total collateral, total borrow balances of the user in ETH.
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            if (vars.compoundedLiquidityBalance > 0) {
                uint256 liquidityBalanceETH = vars
                    .reserveUnitPrice * vars.compoundedLiquidityBalance / vars.tokenUnit;
                ...
                if (vars.usageAsCollateralEnabled && vars.userUsesReserveAsCollateral) {
                    ...
                    currentLtv = currentLtv + liquidityBalanceETH * vars.baseLtv;
                    currentLiquidationThreshold = currentLiquidationThreshold + liquidityBalanceETH * vars.liquidationThreshold;
                }
            }

            if (vars.compoundedBorrowBalance > 0) {
                totalBorrowBalanceETH = totalBorrowBalanceETH + vars.reserveUnitPrice * vars.compoundedBorrowBalance / vars.tokenUnit;
                totalFeesETH = totalFeesETH + vars.originationFee * vars.reserveUnitPrice / vars.tokenUnit;
            }
        }
    ...
    **/
        ...
        returns (bool)
    {
        ...
        vars.liquidationThresholdAfterDecrease = (
            vars.collateralBalanceETH
            * vars.currentLiquidationThreshold 
            - (vars.amountToDecreaseETH * vars.reserveLiquidationThreshold)
            )
   ...
   **/
    function calculateCollateralNeededInETH(
        ...
        uint256 collateralNeededInETH = (_userCurrentBorrowBalanceTH
            + _userCurrentFeesETH + requestedBorrowAmountETH)
            * 100
            / _userCurrentLtv; //LTV is calculated in percentage
    ...
    **/
    function calculateHealthFactorFromBalancesInternal(
        ...
        return
            (collateralBalanceETH * liquidationThreshold / 100).wadDiv(borrowBalanceETH + totalFeesETH);
    }
